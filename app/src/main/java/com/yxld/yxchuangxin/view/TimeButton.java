...
public class TimeButton extends Button implements OnClickListener {
	...
	public void onDestroy() {
		if (AppConfig.map == null)
			AppConfig.map = new HashMap<String, Long>();
		AppConfig.map.put(TIME, time);
		AppConfig.map.put(CTIME, System.currentTimeMillis());
		clearTimer();
	...
	public void onCreate(Bundle bundle) {
		Log.e("yung", AppConfig.map + "");
		if (AppConfig.map == null)
			return;
		if (AppConfig.map.size() <= 0)// 这里表示没有上次未完成的计时
			return;
		long time = System.currentTimeMillis() - AppConfig.map.get(CTIME)
				- AppConfig.map.get(TIME);
		AppConfig.map.clear();
		if (time > 0)
			return;
		else {
