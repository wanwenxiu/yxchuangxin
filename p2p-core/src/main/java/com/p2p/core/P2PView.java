...
//	private RenderThread renderThread;
	...
	private int AutoCruiseOritation=-1;
...
//			renderThread.start();
	...
	public void sendStartBrod(){
		...
		if(!isInitScreen){
			...
			start.setAction(P2PConstants.P2P_WINDOW.Action.P2P_WINDOW_READY_TO_START);
	...
	protected boolean MovePicture(int left, int top) {
		...
		return MediaPlayer.MoveView(left, 0 - top) != 0;
	...
	public void setAutoCruise(int state,int oritation){
		P2PHandler.getInstance().setAutoCruise(state,oritation);
		AutoCruiseOritation=oritation;
	}

	public int getAutoCruiseOritation(){
		return AutoCruiseOritation;
	}
}
