...
//	private static final String UPDATE_URL = "http://gxyaj.cn/upg/android/";
	...
	public boolean checkUpdate(Context mContext,String AppId){
		...
		try{
			//获取当前版本信息
			String version = MyUtils.getVersion(mContext);
...
//			update_version=version_parse[0]+"."+version_parse[1]+"."+vaersion_server_parse[2]+"."+(version_count_server-Integer.parseInt(vaersion_server_parse[2]));
			haveNewVersion = version_count < version_count_server;
		}catch(Exception e){
...
//	}
	...
	public String[] getHelpUrl(Context mContext){
		try {
			//获取当前版本信息
			String version = MyUtils.getVersion(mContext);
			String[] version_parse = version.split("\\.");
			...
			String help_url="";
			String index ="";
			String[] helpUrl=new String[2];
		    while ((line = bReader.readLine()) != null) {  
		        sb.append(line); 
		        if(i==0){
		           help_url=line.substring(line.indexOf("=")+1, line.length());
		           helpUrl[0]=help_url;
		        }
		        if(i==1){
	...
	public String getUpdateDescription(Context mContext,String AppId){
		String description = "";
		BufferedInputStream bis = null;
		try{
		String[] version_parse = MyUtils.getVersion(mContext).split("\\.");
		String url = UPDATE_URL+version_parse[0]+"/"+version_parse[1]+"/des_html.asp?AppId="+AppId;
		URL update_url = new URL(url);
		HttpURLConnection connection = (HttpURLConnection) update_url.openConnection();
	...
	public String getUpdateDescription_en(Context mContext,String AppId){
		String description = "";
		BufferedInputStream bis = null;
		try{
		String[] version_parse = MyUtils.getVersion(mContext).split("\\.");
		String url = UPDATE_URL+version_parse[0]+"/"+version_parse[1]+"/des_html_en.asp?AppId="+AppId;
		URL update_url = new URL(url);
		HttpURLConnection connection = (HttpURLConnection) update_url.openConnection();
		bis = new BufferedInputStream(connection.getInputStream());
		StringBuffer desBuffer = new StringBuffer();
	...
	public void downloadApk(Context mContext,Handler handler,String filePath,String fileName){
