...
public class NetManager {
    ...
    private static String ALARM_MSSAGE_URL = "http://192.168.1.222/"
            + "Alarm/AlarmRecordEx.ashx";
    private static String SYSTEM_MESSAGE_URL =SERVER1 +
            "business/seller/recommendinfo.ashx";
    private static String LOGO_URL = SERVER1 +
            "AppInfo/getappstartinfo.ashx";
    private static String MALL_URL = SERVER1 +
            "AppInfo/getstorelinks.ashx";
    private static String STORE_ID_URL = SERVER1 +
            "AppInfo/SetStoreID.ashx";
    //邮箱找回密码
    private static String SEND_EMAIL_BACK = SERVER1 + "Password/GetAccountByEmail.ashx";
    // 重置密码
    private static String PASS_RESETPWD = SERVER1
            + "Password/ResetPWD.ashx";
    private static String GET_ACCOUNT_PHONENO = SERVER1
            + "Password/GetAccountByPhoneNO.ashx";
    private static String BACK_CHECK_PHONE_VKEY = SERVER1
            + "Password/CheckPhoneVKey.ashx";
    private static String THIRD_LOGIN = SERVER1 +
            "Users/ThirdLogin.ashx";
...
//		SYSTEM_MESSAGE_URL=server+"business/seller/recommendinfo.ashx";
        ACCOUNT_INFO_EXIST = server+"Account/IsAccountExisted.ashx";
        THIRD_LOGIN=server+"Users/ThirdLogin.ashx";
    }

    //添加公共参数
    private void setPublicParames(List<NameValuePair> params){
        String appID= SharedPrefreUtils.getInstance().getStringData(mContext, P2PSpecial.P2P_APPID);
        String token= SharedPrefreUtils.getInstance().getStringData(mContext, P2PSpecial.P2P_APPTOKEN);

        params.add(new BasicNameValuePair("AppID", appID));
        params.add(new BasicNameValuePair("AppToken", token));
        params.add(new BasicNameValuePair("Language", MyUtils.getLanguage(mContext)));
        params.add(new BasicNameValuePair("AppOS", "3"));
        params.add(new BasicNameValuePair("AppName", mContext.getPackageName()));
        params.add(new BasicNameValuePair("AppVersion", MyUtils.getBitProcessingVersion(mContext)));
        params.add(new BasicNameValuePair("PackageName", mContext.getPackageName()));
        params.add(new BasicNameValuePair("ApiVersion", Config.AppConfig.apiversion));
    }
    //是否需要更换服务器
    private boolean NeedChangeServer(String result){
        ...
        try {
            ...
            if("10901040".equals(error)||"109013E9".equals(error)){
                return true;
            }
            String errorcode = obj.getString("error_code");
            int error_code=Integer.parseInt(errorcode);
            if(error_code==1||error_code==29||error_code==404||error_code==500||error_code==998
                    ||error_code==999||error_code==1001){
                return true;
            }
    ...
    public String doPostForThirdLogin(List<NameValuePair> params, String url)
            throws Exception {
        String appID= SharedPrefreUtils.getInstance().getStringData(mContext, P2PSpecial.P2P_APPID);
        String token= SharedPrefreUtils.getInstance().getStringData(mContext, P2PSpecial.P2P_APPTOKEN);
        if (!isInit) {
        ...
        params.add(new BasicNameValuePair("AppVersion", MyUtils.getBitProcessingVersion(mContext)));
        params.add(new BasicNameValuePair("PackageName", mContext.getPackageName()));
        params.add(new BasicNameValuePair("ApiVersion", "1"));
        String result = null;
        // 新建HttpPost对象
        HttpPost httpPost = new HttpPost(url);
        // 设置字符集
    ...
    public int getAccountExist(String account) {
        int result = UNKNOWN_ERROR;
        List<NameValuePair> params = new ArrayList<NameValuePair>();
    ...
    public JSONObject sendEmail(String email) {
        JSONObject jObject = null;
        String BodyField1 = "亲爱的";
        String BodyField2 = "，您好：<br>您申请重置账号密码，如非本人操作，请忽略此邮件。<br>请点击下面链接来重置您的登录密码:";
        String BodyField3 = "<br>为了确保您的帐号安全，该链接仅60分钟内访问有效。<br>如果上面的链接无法点击，您可以复制并粘帖到浏览器的地址栏中访问。<br>";
        List<NameValuePair> params = new ArrayList<NameValuePair>();
//        params.add(new BasicNameValuePair("BodyField1", BodyField1));
//        params.add(new BasicNameValuePair("BodyField2", BodyField2));
//        params.add(new BasicNameValuePair("BodyField3", BodyField3));
        params.add(new BasicNameValuePair("Email", email));
        try {
            jObject = new JSONObject(doPost(params, SEND_EMAIL_BACK));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return jObject;
    ...
    // 重置密码
    public JSONObject resetPwd(String ID, String VKey, String NewPwd,
                               String ReNewPwd) {
        JSONObject jObject = null;
        List<NameValuePair> params = new ArrayList<NameValuePair>();
        MD5 md = new MD5();
        ...
        params.add(new BasicNameValuePair("NewPwd", md.getMD5ofStr(NewPwd)));
        params.add(new BasicNameValuePair("ReNewPwd", md.getMD5ofStr(ReNewPwd)));
        try {
            jObject = new JSONObject(doPost(params, PASS_RESETPWD));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return jObject;
    ...
    public JSONObject getAccountByPhoneNO(String CountryCode, String PhoneNO) {
        JSONObject jObject = null;
        List<NameValuePair> params = new ArrayList<NameValuePair>();
        params.add(new BasicNameValuePair("CountryCode", CountryCode));
        params.add(new BasicNameValuePair("PhoneNO", PhoneNO));
        try {
            jObject = new JSONObject(doPost(params, GET_ACCOUNT_PHONENO));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return jObject;
    ...
    // 验证手机验证码
    public JSONObject getCheckPhoneVKey(String ID, String VKey,
                                        String CountryCode, String PhoneNO, String PhoneVerifyCode) {
        JSONObject jObject = null;
        List<NameValuePair> params = new ArrayList<NameValuePair>();
        params.add(new BasicNameValuePair("ID", ID));
        params.add(new BasicNameValuePair("VKey", VKey));
        params.add(new BasicNameValuePair("CountryCode", CountryCode));
        params.add(new BasicNameValuePair("PhoneNO", PhoneNO));
        params.add(new BasicNameValuePair("PhoneVerifyCode", PhoneVerifyCode));
        try {
            jObject = new JSONObject(doPost(params, BACK_CHECK_PHONE_VKEY));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return jObject;
    ...
    // 手机重置密码验证信息的对象转换
    public static CheckPhoneVKeyResult setCheckPhoneVKeyResult(
            JSONObject jObject) {
        CheckPhoneVKeyResult result = null;
        result = new CheckPhoneVKeyResult(jObject);
        return result;
    }

    public static GetAccountByPhoneNOResult creatAccountByPhoneNOResult(
            JSONObject jObject) {
        GetAccountByPhoneNOResult result = null;
        result = new GetAccountByPhoneNOResult(jObject);
        return result;
    }
    ...
    /**
     * 第三方登陆
     * @param option 1.登陆 2.绑定 3.新用户
     * @param UnionID 第三方唯一标识
     * @param User 用户ID
     * @param UserPwd 用户密码
     * @param StoreID 商城id
     ...
     * @return
     */
    public JSONObject ThirdLogin(String option,String UnionID,String User, String UserPwd,String StoreID,String ThirdType) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        JSONObject jObject = null;
        List<NameValuePair> params = new ArrayList<NameValuePair>();
        MD5 md = new MD5();
        UnionID= SHA.SHA256(UnionID);
        params.add(new BasicNameValuePair("Option", option));
        params.add(new BasicNameValuePair("PlatformType", ThirdType));
        params.add(new BasicNameValuePair("UnionID", UnionID));
        params.add(new BasicNameValuePair("Sign", md.getMD5ofStr(UnionID+ThirdType+UnionID)));

        if(!TextUtils.isEmpty(User)){
            params.add(new BasicNameValuePair("User", User));
        }
        if(!TextUtils.isEmpty(UserPwd)){
            UserPwd = md.getMD5ofStr(UserPwd);
        ...
        try {
            jObject = new JSONObject(doPostForThirdLogin(params, THIRD_LOGIN));
        } catch (Exception e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
    ...
    public static ThirdPartyLoginResult createThirdPartyLoginResult(JSONObject jObject) {
        ThirdPartyLoginResult result = null;
        result = new ThirdPartyLoginResult(jObject);
        return result;
    }
}
