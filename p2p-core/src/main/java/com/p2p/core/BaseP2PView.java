...
//    	if(mgapLenght>0){
//    		P2PHandler.getInstance().setZoom(P2PConstants.ZOOM.ZOOM_BIG);
//    	}else{
//    		P2PHandler.getInstance().setZoom(P2PConstants.ZOOM.ZOOM_SMALL);
//    	}
    	int multiple=0;
    	if(mgapLenght<-600){
    		multiple=-4;
	...
	void setScale(float scale) {
		...
		if (scale > 1 && this.getWidth() <= this.getMaxWidth()) {
			...
			isControl_V = this.getHeight() > screen_H;

			isControl_H = this.getWidth() > screen_W;
